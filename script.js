...
// === ДАННЫЕ ПРОДУКТОВ (полные описания) ===
const products = {
  "OGSM": {
...
function generateReport(businessProfile, financeProfile) {
  if (financeProfile === "F1" || financeProfile === "F2" || financeProfile === "F3") {
    const productKey = financeProfile === "F1" ? "F1" : financeProfile === "F2" ? "F2" : "F3";
    return {
      diagnosis: products[productKey].full.split('\n')[0],
...
function showQuestion(step) {
  ...
  q.options.forEach(opt => {
    ...
    html += `
      <label>
        <input type="radio" name="q${q.id}" value="${opt.value}" ${checked}> ${opt.text}
      </label>`;
  });
  html += `</div>`;
